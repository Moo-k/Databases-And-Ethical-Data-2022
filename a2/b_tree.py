...
class BTree:
    ...
    def search_key(self, k, x = None):
        ...
        # ... your code here
        if x is None: # if x is None (first call), set x to the root of the B-tree
            x = self.root
            return self.search_key(k,x)
        else:
            i = 0
            while i < len(x.key_vals) and k > x.key_vals[i][0]: # find k in key_vals by incrementing the index in the node
                i += 1
            if i < len(x.key_vals) and k == x.key_vals[i][0]: # if found, return node and index
                return (x, i)
            else: # if k is not found and the node has children nodes, call the search_key function on children
                if i < len(x.children):
...
def linear_search(array, query_value):
    ...
    # ... your code here
    results = []
    for i in range(0,len(array)-1): # for each index in the array
        if array[i] == query_value: # if the query value matches the element
            results.append(i) # add the index to the output
    return results
...
# Limitations of this evaluation: This evaluation does not account for addition or removal of data. The time taken to add or remove values from the linear search array would be less than that of the B-tree.